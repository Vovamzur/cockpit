    ...
    if (this.props.masked) {
      actions.push(
        <DropdownItem
          key="unmask"
          onClick={() =>
            this.props.fileActionCallback("UnmaskUnitFiles", undefined)
          }
        >
          {_("Allow running (unmask)")}
        </DropdownItem>
      );
    } else {
      // All cases when not masked
      if (this.props.active) {
        if (this.props.canReload) {
          actions.push(
            <DropdownItem
              key="reload"
              onClick={() => this.props.actionCallback("ReloadUnit")}
            >
              {_("Reload")}
            </DropdownItem>
          );
        ...
        actions.push(
          <DropdownItem
            key="restart"
            onClick={() => this.props.actionCallback("RestartUnit")}
          >
            {_("Restart")}
          </DropdownItem>
        );
        actions.push(
          <DropdownItem
            key="stop"
            onClick={() => this.props.actionCallback("StopUnit")}
          >
            {_("Stop")}
          </DropdownItem>
        );
        ...
        actions.push(
          <DropdownItem
            key="start"
            onClick={() => this.props.actionCallback("StartUnit")}
          >
            {_("Start")}
          </DropdownItem>
        );
      ...
      if (actions.length > 0) {
        actions.push(<DropdownSeparator key="divider" />);
      }
      ...
      if (this.props.failed) {
        actions.push(
          <DropdownItem
            key="reset"
            onClick={() => this.props.actionCallback("ResetFailedUnit", [])}
          >
            {_("Clear 'Failed to start'")}
          </DropdownItem>
        );
      }
      ...
      actions.push(
        <DropdownItem
          key="mask"
          onClick={() => this.setState({ dialogMaskedOpened: true })}
        >
          {_("Disallow running (mask)")}
        </DropdownItem>
      );
    }
        ...
        <Dropdown
          ...
          toggle={
            <KebabToggle
              ...
              onToggle={(isActionOpen) => this.setState({ isActionOpen })}
            />
          }
          isOpen={this.state.isActionOpen}
          isPlain
          onSelect={() =>
            this.setState({ isActionOpen: !this.state.isActionOpen })
          }
...
export class ServiceDetails extends React.Component {
  constructor(props) {
    ...
    this.unitTypeCapitalized =
      this.unitType.charAt(0).toUpperCase() + this.unitType.slice(1);
  ...
  doMemoryCurrentPolling() {
    systemd_client
      .call(this.props.unit.path, "org.freedesktop.DBus.Properties", "Get", [
        "org.freedesktop.systemd1." + this.unitTypeCapitalized,
        "MemoryCurrent",
      ])
      .then(
        (result) => {
  ...
  addUnitProperties(prop, value) {
    if (
      prop == "MemoryCurrent" &&
      this.state.unit_properties.MemoryCurrent !== value
    ) {
      this.setState({
        unit_properties: Object.assign(this.state.unit_properties, {
          [prop]: value,
        }),
      });
    }
  ...
  onOnOffSwitch() {
    if (this.props.unit.UnitFileState === "enabled") {
      ...
      if (
        this.props.unit.ActiveState === "active" ||
        this.props.unit.ActiveState === "activating"
      ) {
  ...
  unitAction(method, extra_args) {
    if (extra_args === undefined) extra_args = ["fail"];
    ...
    systemd_client
      .call(
        ...
        SD_MANAGER,
        method,
        [this.props.unit.Names[0]].concat(extra_args)
      )
      .catch((error) =>
        this.setState({ error: error.toString(), waitsAction: false })
      );
  }
  ...
  unitFileAction(method, force) {
    ...
    if (force !== undefined) args.push(force == "true");
    systemd_client
      .call(SD_OBJ, SD_MANAGER, method, args)
      .then(([results]) => {
        if (results.length == 2 && !results[0]) {
          this.setState({
            note: _("This unit is not designed to be enabled explicitly."),
          });
        }
  ...
  render() {
    ...
    if (masked) {
      status.push(
        <div key="masked" className="status-masked">
          ...
          <span className="side-note font-xs">
            {_("Forbidden from running")}
          </span>
        </div>
      );
    }
        ...
        <div key="failed" className="status-failed">
          ...
          {this.props.permitted && (
            <Button
              ...
              onClick={() => this.unitAction("StartUnit")}
            >
              {_("Start service")}
            </Button>
          )}
      ...
      if (active) {
        status.push(
          <div key="running" className="status-running">
            ...
            <span className="side-note font-xs">
              {_("Active since ") +
                moment(this.props.unit.ActiveEnterTimestamp / 1000).format(
                  "LLL"
                )}
            </span>
          </div>
          ...
          {this.props.unit.WantedBy && this.props.unit.WantedBy.length > 0 && (
            <>
              ...
              <ul className="comma-list">
                {this.props.unit.WantedBy.map((unit) => (
                  <li className="font-xs" key={unit}>
                    <a href={"#/" + unit}>{unit}</a>
                  </li>
                ))}
              </ul>
    ...
    if (!this.props.permitted) {
      status.unshift(
        <div key="readonly" className="status-readonly">
          ...
          <span className="side-note font-xs">
            {_("Requires administration access to edit")}
          </span>
        </div>
      );
    }
    ...
    const loadError = this.props.unit.LoadError
      ? this.props.unit.LoadError[1]
      : null;
    ...
    const relationships = [
      ...
      {
        Name: _("Propagates reload to"),
        Units: this.props.unit.PropagatesReloadTo,
      },
      {
        Name: _("Reload propagated from"),
        Units: this.props.unit.ReloadPropagatedFrom,
      },
      {
        Name: _("Joins namespace of"),
        Units: this.props.unit.JoinsNamespaceOf,
      },
    ];

    const relationshipsToList = (rels) => {
      return rels
        .filter((rel) => rel.Units && rel.Units.length > 0)
        .map((rel) => (
                ...
                {rel.Units.map((unit) => (
                  <li key={unit}>
                    <Button
                      ...
                      component="a"
                      href={"#/" + unit}
                      isDisabled={!this.props.isValid(unit)}
                    >
                      {unit}
                    </Button>
                  </li>
    ...
    const extraRelationshipsList = (
      <ExpandableSection
        id="service-details-show-relationships"
        toggleText={
          triggerRelationshipsList.length
            ? _("Show more relationships")
            : _("Show relationships")
        }
      >
    ...
    if (conditions) {
      conditions.map((condition) => {
        if (condition[4] < 0) {
          notMetConditions.push(
            cockpit.format(
              _("Condition $0=$1 was not met"),
              condition[0],
              condition[3]
            )
          );
      ...
      <Card>
        {(this.state.note || this.state.error) && (
          <ServiceConfirmDialog
            ...
            close={() =>
              this.setState(this.state.error ? { error: "" } : { note: "" })
            }
          />
        )}
        {hasLoadError && this.props.unit.LoadState ? (
          ...
          <>
            <CardTitle className="service-top-panel">
              <Text component={TextVariants.h2} className="service-name">
              ...
              {this.props.permitted && (
                <>
                    ...
                    >
                      <span>
                        <Switch
                          ...
                          isDisabled={
                            this.state.waitsAction || this.state.waitsFileAction
                          }
                  ...
                  <ServiceActions
                    {...{ active, failed, enabled, masked }}
                    canReload={this.props.unit.CanReload}
                    actionCallback={this.unitAction}
                    fileActionCallback={this.unitFileAction}
                    disabled={
                      this.state.waitsAction || this.state.waitsFileAction
                    }
                  />
            ...
            <CardBody>
              <DescriptionList isHorizontal>
                ...
                <DescriptionListGroup>
                  ...
                  <DescriptionListDescription id="path">
                    {this.props.unit.FragmentPath}
                  </DescriptionListDescription>
                </DescriptionListGroup>
                {unit.MemoryCurrent ? (
                  <DescriptionListGroup>
                    <DescriptionListTerm>{_("Memory")}</DescriptionListTerm>
                    <DescriptionListDescription id="memory">
                      {cockpit.format_bytes(unit.MemoryCurrent, 1024)}
                    </DescriptionListDescription>
                  </DescriptionListGroup>
                ) : null}
                {this.props.unit.Listen && this.props.unit.Listen.length && (
                  <DescriptionListGroup>
                    <DescriptionListTerm>{_("Listen")}</DescriptionListTerm>
                    <DescriptionListDescription id="listen">
                      {cockpit.format(
                        "$0 ($1)",
                        this.props.unit.Listen[0][1],
                        this.props.unit.Listen[0][0]
                      )}
                    </DescriptionListDescription>
                  </DescriptionListGroup>
                )}
                {notMetConditions.length > 0 && (
                  <DescriptionListGroup>
                    <DescriptionListTerm className="failed">
                      {_("Condition failed")}
                    </DescriptionListTerm>
                    <DescriptionListDescription id="condition">
                      {notMetConditions.map((cond) => (
