...
const CreateTimerDialogBody = ({ setIsOpen }) => {
  ...
  if (!description.trim().length) validationFailed.description = true;
  if (!command.trim().length) validationFailed.command = true;

  const timePicker = (idx) => (
    <TimePicker
      className="create-timer-time-picker"
      defaultTime="00:00"
      is24Hour
    ...
    <Modal
      ...
      footer={
        <>
          {dialogError && (
            <ModalError
              dialogError={_("Timer creation failed")}
              dialogErrorDetail={dialogError}
            />
          )}
          <Button
            variant="primary"
            ...
            onClick={() => {
              ...
              if (Object.keys(validationFailed).length) return;
    ...
    >
      <Form isHorizontal>
        <FormGroup
          ...
          helperTextInvalid={
            !name.trim().length
              ...
              : _("Only alphabets, numbers, : , _ , . , @ , - are allowed")
          }
        >
          <TextInput
            id="servicename"
        ...
        <FormGroup
          ...
          fieldId="description"
          validated={
            submitted && validationFailed.description ? "error" : "default"
          }
        ...
        <FormGroup
          ...
          fieldId="command"
          validated={
            submitted && validationFailed.command ? "error" : "default"
          }
          helperTextInvalid={_("This field cannot be empty")}
        >
          <TextInput
        ...
        <FormGroup label={_("Run")}>
          ...
          {delay == "system-boot" && (
            <FormGroup className="service-run delay-group" label={_("After")}>
              <TextInput
                value={delayNumber}
                validated={
                  submitted && validationFailed.delayNumber
                    ? "error"
                    : "default"
                }
          ...
          {delay == "specific-time" && (
            <FormGroup className="service-run" label={_("Repeat")}>
              <FormSelect
                value={repeat}
                id="drop-repeat"
                onChange={(value) => {
                  ...
                  if (value == "hourly")
                    ...
                    setRepeatPatterns([
                      { key: 0, date: undefined, time: "00:00" },
                    ]);
                }}
                aria-label={_("Repeat")}
              >
              ...
              {repeat == "no" && (
                <TimePicker
                  className="create-timer-time-picker specific-no-repeat"
                  menuAppendTo={() => document.body}
                  defaultTime="00:00"
                  is24Hour
              ...
              {repeatPatterns.map((item, idx) => {
                ...
                return (
                  <div
                    className="specific-repeat-group"
                    data-index={idx}
                    key={item.key}
                  >
                    {repeat == "hourly" && (
                      <>
                        <TextInput
                          ...
                          validated={
                            submitted && validationFailedMinute
                              ? "error"
                              : "default"
                          }
                        />
                        {submitted && validationFailedMinute && (
                          <FormHelperText isError isHidden={false}>
                            {_("Minute needs to be a number between 0-59")}
                          </FormHelperText>
                        )}
                      </>
                    )}
                    ...
                    {repeat !== "no" && (
                      <span className="action-group">
                        <Button
                          ...
                          onClick={() =>
                            setRepeatPatterns(
                              repeatPatterns.filter(
                                (item, item_idx) => idx != item_idx
                              )
                            )
                          }
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (repeat == "hourly")
                              setRepeatPatterns([
                                ...repeatPatterns,
                                { key: repeatPatterns.length, minute: "0" },
                              ]);
                            else if (repeat == "daily")
                              setRepeatPatterns([
                                ...repeatPatterns,
                                { key: repeatPatterns.length, time: "00:00" },
                              ]);
                            else if (repeat == "weekly")
                              setRepeatPatterns([
                                ...repeatPatterns,
                                {
                                  key: repeatPatterns.length,
                                  day: "mon",
                                  time: "00:00",
                                },
                              ]);
