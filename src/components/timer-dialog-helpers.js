...
export function create_timer({
  ...
  if (delay == "specific-time" && repeat == "no") {
    ...
    timer_unit.OnCalendar =
      "OnCalendar=*-*-* *:" + timer_unit.repeat_minute.join(",");
  ...
  if (repeat != "hourly" && delay == "specific-time")
    timer_unit.OnCalendar = timer_unit.OnCalendar.toString().replace(
      /,/g,
      "\n"
    );
...
function create_timer_file({ timer_unit, delay }) {
  ...
  var service_file =
    unit + timer_unit.Description + service + timer_unit.Command + "\n";
  var timer_file = " ";
  if (delay == "system-boot") {
    var boottimer =
      timer +
      "OnBootSec=" +
      timer_unit.boot_time +
      timer_unit.boot_time_unit +
      "\n";
  ...
  file.replace(service_file).catch((error) => console.log(error.toString()));
  var timer_path = "/etc/systemd/system/" + timer_unit.name + ".timer";
  file = cockpit.file(timer_path, { superuser: "try" });
  return file
    .replace(timer_file)
    .then((tag) => {
      return systemd_client.call(SD_OBJ, SD_MANAGER, "EnableUnitFiles", [
    ...
    .then(() => {
      ...
      if (timer_unit.OnCalendar)
        return systemd_client.call(SD_OBJ, SD_MANAGER, "StartUnit", [
          timer_unit.name + ".timer",
          "replace",
        ]);
    });
